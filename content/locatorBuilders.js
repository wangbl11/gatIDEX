...
LocatorBuilders.prototype.clickLabel = function(e) {
  if (e.nodeName) {
    ...
    if (nodeName == "label") {
      if (e.hasAttribute && e.hasAttribute("for")) {
        //gat-3284
...
LocatorBuilders.prototype.existingCategory = function(
  ary,
  finderName,
  category,
  locator
...
LocatorBuilders.prototype.lastConfirm = function(
  e,
  _main,
  locator,
  coreLocatorStrategies,
  finderName,
  _category,
  _main1
) {
  if (finderName != "tac") {
    ...
    if (
      e == fe ||
      (coreLocatorStrategies[finderName] &&
        coreLocatorStrategies[finderName].is_fuzzy_match &&
...
LocatorBuilders.prototype.getNodeCoords = function(
  node,
  bAdjusted,
  bNodeContents
) {
...
LocatorBuilders.checkDynamicID = function(txt) {
  for (var ii = 0; ii < LocatorBuilders._dynamicIDs.length; ii++) {
    if (txt.match(LocatorBuilders._dynamicIDs[ii])) {
      return true;
    }
  }
  return false;
...
LocatorBuilders.prototype.getElementDataXPath = function(element, rootNode) {
  ...
  try {
    ...
    if (result.dataRelated) {
      xpaths = this.genUniqueXPathsFromDataRoot(
        element,
        result.dataRootElement,
...
LocatorBuilders.prototype.checkIfDataRelated = function(element, rootNode) {
  var ret = {
    dataRelated: false,
    dataRootElement: null,
    dataRootClassName: null
  };
...
LocatorBuilders.prototype.genUniqueXPathsFromDataRoot = function(
  element,
  dataRootElement,
  dataRootClassName
) {
  ...
  for (var i = 0; i < uniqueTexts.length; i++) {
    ...
    var xpath = LocatorBuilders.DATA_ROOT_XPATH.replace(
      "DATA_ROOT_TAG",
      dataRootElement.tagName
    )
      .replace("DATA_ROOT_CLASS_NAME", dataRootClassName)
      .replace("UNIQUE_TEXT", uniqueTexts[i]);
    if (element.className) {
      xpath =
        xpath + "//" + element.tagName + "[@class='" + element.className + "']";
    } else {
      //actually we can always use this branch, to check again in future
      xpath =
        xpath + this.getElementTreeXPathFromDataRoot(element, dataRootElement);
    }
    if (this.getElementCount("xpath", xpath) == 1) {
      xpaths.push(xpath);
    }
    if (xpaths.length == 5) {
      //only found at most 5
      break;
    }
  }
  return xpaths;
...
LocatorBuilders.prototype.getElementTreeXPathFromDataRoot = function(
  element,
  dataRootElement
) {
  var paths = [];
  for (
    ;
    element != dataRootElement && element.nodeType == 1;
...
LocatorBuilders.prototype.getElementCount = function(using, locator) {
  ...
  try {
    ...
    console.log(
      "[ERROR] getElementCount: Generated selector is wrong: using = " +
        using +
        ", locator = " +
        locator
    );
    console.log(
      "[ERROR] getElementCount: Generated selector is wrong with exception: " +
        ex
    );
  }
  return elemNum;
...
LocatorBuilders.prototype.getCSSSubPath = function(e) {
  ...
  for (var i = 0; i < css_attributes.length; i++) {
    ...
    if (value) {
      if (attr == "id") {
        for (var ii = 0; ii < LocatorBuilders._dynamicIDs.length; ii++) {
          if (value.match(LocatorBuilders._dynamicIDs[ii])) {
            return null;
          }
        }
        return "#" + value;
...
LocatorBuilders.prototype.ancestorText = function(e) {
  ...
  if (txt != null)
    return (
      "//" +
      _nodeName +
      (txt.indexOf("descendant") < 0 ? "[" : "[descendant::") +
      txt +
      "]"
    );
...
LocatorBuilders.prototype.inputByLabelCheck = function(e) {
  ...
  if (_nodeName == "input" && e.hasAttribute && e.hasAttribute("id")) {
    ...
    if (labelElement) {
      ...
      if (txt && txt.length > 0) {
        ret =
          "//label[normalize-space(text())='" + txt + "']/following::input[1]";
...
LocatorBuilders.prototype.getParentDivElement = function(elem) {
  ...
  while (tmpNode) {
    if (parentTags.indexOf(tmpNode.localName) >= 0) break;
    tmpNode = tmpNode.parentNode;
  }
...
LocatorBuilders.operatorPriority = [
  "equals",
  "normalize-space",
  "attrs",
  "contains"
...
LocatorBuilders.prototype.genPredicateWithText = function(
  result,
  path,
  textItem,
  depth
  ...
  var text = textItem.text,
    operator = textItem.operator;
  if (text.match(/\'/) && text.match(/\"/)) return; // We skip it in case the text contains both single quote and double quote
  var predicate = "";
  if (text.match(/\'/)) {
    if (operator == LocatorBuilders.TEXT_XPATH_OPERATOR_EQUALS)
      predicate = LocatorBuilders.TEXT_XPATH_EQUALS1.replace(
        /TEXTVALUE/g,
        text
      );
    else if (operator == LocatorBuilders.TEXT_XPATH_OPERATOR_NORMALIZE)
      predicate = LocatorBuilders.TEXT_XPATH_NORMALIZE1.replace(
        /TEXTVALUE/g,
        text
      );
    else
      predicate = LocatorBuilders.TEXT_XPATH_CONTAINS1.replace(
        /TEXTVALUE/g,
        text
      );
  } else {
    if (operator == LocatorBuilders.TEXT_XPATH_OPERATOR_EQUALS)
      predicate = LocatorBuilders.TEXT_XPATH_EQUALS2.replace(
        /TEXTVALUE/g,
        text
      );
    else if (operator == LocatorBuilders.TEXT_XPATH_OPERATOR_NORMALIZE)
      predicate = LocatorBuilders.TEXT_XPATH_NORMALIZE2.replace(
        /TEXTVALUE/g,
        text
      );
    else
      predicate = LocatorBuilders.TEXT_XPATH_CONTAINS2.replace(
        /TEXTVALUE/g,
        text
      );
  }
  //this.logger.info('[DEBUG] predicate: ' + predicate);
  var elemTag = textItem.isSVG
    ? "*[name()='" + textItem.tag + "']"
    : textItem.tag;
  var xpath = path
    .replace(/TAG/g, elemTag)
    .replace(/\[PREDICATE\]/g, predicate);
  if (depth == 1)
    xpath = path
      .replace(/AXIS/g, "child")
      .replace(/TAG/g, elemTag)
      .replace(/\[PREDICATE\]/g, predicate);
  else if (depth >= 2)
    xpath = path
      .replace(/AXIS/g, "child")
      .replace(/TAG/g, "*")
      .replace(/\[PREDICATE\]/g, "[descendant::" + elemTag + predicate + "]");
  var item = {
    tag: textItem.tag,
    text: text,
    operator: operator,
...
LocatorBuilders.prototype.sortTextXPathResult = function(result) {
  result.sort(function(a, b) {
    var opResult =
      LocatorBuilders.operatorPriority.indexOf(a.operator) -
      LocatorBuilders.operatorPriority.indexOf(b.operator);
    if (opResult > 0) {
      return 1;
    } else if (opResult < 0) {
      return -1;
...
LocatorBuilders.prototype.generateTextXPath = function(element) {
  ...
  var suffix = "";
  if (nodeNbrs > -1) suffix = "[" + (nodeNbrs + 1) + "]";
  ...
  if (textItem) {
    ...
    this.genPredicateWithText(
      result,
      "//TAG" + suffix + "[PREDICATE]",
      textItem,
      0
    );
  }
  ...
  var elemTag = isSVG
    ...
    : this.xpathHtmlElement(element.nodeName.toLowerCase());
  elemStack.push({
    node: element,
  ...
  while (elemItem && !enough) {
    ...
    for (
      var childElem = currentElem.firstElementChild;
      childElem;
...
LocatorBuilders.add(
  ...
  function(e) {
    ...
    if (nodeName == "polygon") {
      ...
      if (e.hasAttribute) {
        let _d = e.getAttribute("fill");
        if (_d) {
          return this.preciseXPath(
            "//*[local-name()='path' and @fill='" + _d + "']",
            e
          );
        }
      }
  ...
  function(e) {
    ...
    if (e.id) {
      ...
      for (var i = 0; i < LocatorBuilders._validIDs.length; i++) {
        if (_id.match(LocatorBuilders._validIDs[i])) {
          return "id=" + e.id;
        }
      }

      for (var i = 0; i < LocatorBuilders._dynamicIDs.length; i++) {
        if (_id.match(LocatorBuilders._dynamicIDs[i])) {
          dyId = true;
          break;
        }
      }
      if (dyId == false) return "id=" + e.id;
    }
    return null;
  },
  "id"
...
LocatorBuilders.add(
  ...
  function(e) {
    ...
    if (_name) {
      for (var i = 0; i < LocatorBuilders._dynamicIDs.length; i++) {
        if (_name.match(LocatorBuilders._dynamicIDs[i])) {
          dyName = true;
          break;
        }
      }
      if (dyName == false) return "name=" + e.name;
    }
    return null;
  },
  "name"
...
LocatorBuilders.add(
  ...
  function(e) {
    ...
    while (
      ...
      else sub_path = cur_path + " > " + sub_path;
      current = current.parentNode;
      level++;
    }
    //console.log(sub_path);
    ...
    if (TEXT_TAGS.includes(_nodeName) && e.hasAttribute) {
      ...
      if (_text != null) {
        ...
        var _text2 =
          ...
          _extra +
          _text +
          " and not(ancestor::div[contains(@style,'display:none')]) and not(ancestor::div[contains(@style,'display: none')])]";
        //gat-4132
      ...
      for (var u = 0; u < ELEM_TEXT_ATTRIBUTES.length; u++) {
        if (e.hasAttribute(ELEM_TEXT_ATTRIBUTES[u])) {
          var _e = this.preciseXPath(
            "//" +
              _nodeName +
              "[@" +
              ELEM_TEXT_ATTRIBUTES[u] +
              "=" +
              this.attributeValue(e.getAttribute(ELEM_TEXT_ATTRIBUTES[u])) +
              "]",
            e
          );
...
LocatorBuilders.prototype.buildForWhat = function(
  type,
  name,
  e,
  opt_contextNode
